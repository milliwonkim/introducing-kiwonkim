...
const NAV_ITEMS = [
  ...
  { label: "Recent Projects", id: "projects" },
  { label: "Tech Blog", id: "blog" },
  ...
  return (
    <header
      className={clsx(
        ...
        isScrolled
          ? "backdrop-blur bg-[color:var(--color-background)]/90 shadow-[0_10px_30px_rgba(15,23,42,0.12)]"
          : "bg-transparent"
      )}
    >
      <div className="mx-auto flex h-[var(--header-height,4.5rem)] max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden md:flex items-center gap-0.5 rounded-full border border-[color:var(--color-border-light)] bg-[color:var(--color-card-background)]/80 px-1.5 py-1 backdrop-blur">
          {NAV_ITEMS.map((item) => {
